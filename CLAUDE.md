...
- **Dexie/IndexedDB** (`src/db/db.ts`, table `meals`) holds locally-logged food entries, queried live via `dexie-react-hooks`' `useLiveQuery`. This is offline-first and never synced to Firestore — meal history is local to the device only.
